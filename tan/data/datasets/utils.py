...
def moment_to_stiou2d(gttube, tube, num_clips):
    stiou2d = torch.ones(num_clips, num_clips)
    grids = stiou2d.nonzero()
    grids[:, 1] += 1
    moments = grids / num_clips
    tube_len = tube.size(0)
    cadidates = torch.round(moments * tube_len).long()
    for i in range(cadidates.size(0)):
        st = cadidates[i][0]
        ed = cadidates[i][1]
        if st < ed:
            subtube = tube[st:ed]
            stiou2d[i // num_clips][i % num_clips] = iou3dt(gttube.numpy(), subtube.numpy())
        else:
            stiou2d[i // num_clips][i % num_clips] = 0
    return stiou2d
...
def get_tube_feat(feat, num_pre_clips):
...
def get_i3d_charades(feat_file, num_pre_clips):
    assert exists(feat_file)
    feat = torch.from_numpy(np.load(feat_file)).squeeze(1).squeeze(1)
    #feat = F.normalize(feat, dim=1)
    return maxfeats(feat, num_pre_clips)
